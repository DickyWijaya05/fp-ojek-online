...
<main class="p-6 max-w-7xl mx-auto space-y-10">
  ...
  @php
    $stats = [
      ['title' => 'Total Pengguna', 'value' => '1.203', 'icon' => 'users', 'color' => 'from-indigo-400 to-indigo-600', 'route' => route('admin.users')],
      ['title' => 'Total Driver', 'value' => '342', 'icon' => 'bike', 'color' => 'from-teal-400 to-teal-600', 'route' => route('admin.drivers.index')],
  ...
  <!-- Welcome Card -->
  <div class="bg-white p-6 rounded-2xl shadow-md border-l-4 border-yellow-400">
    <h2 class="text-xl font-semibold text-yellow-600 mb-1">👋 Selamat Datang, {{ Auth::user()->name }}</h2>
    <p class="text-gray-700">Anda login sebagai <strong>{{ Auth::user()->email }}</strong>. Dashboard ini dirancang untuk membantu Anda memantau sistem dengan mudah.</p>
  </div>

  <!-- Filter & Chart Loader -->
  <div class="flex justify-between items-center">
    <h3 class="text-lg font-semibold text-gray-800">Statistik Aktivitas</h3>
    <select id="filterWaktu" class="border rounded-md px-4 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-400">
  ...
  <!-- Chart Loader -->
  <div id="chartLoader" class="flex items-center justify-center h-64 w-full bg-gray-50 rounded-xl">
