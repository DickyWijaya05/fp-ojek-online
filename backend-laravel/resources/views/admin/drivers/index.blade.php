...
@section('content')
  <main class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <button onclick="document.getElementById('modalTambahDriver').classList.remove('hidden')" class="inline-flex items-center gap-2 px-4 py-2 
    ...
    <div class="flex items-center gap-2">
      ...
      <input id="searchInput" type="text" placeholder="Cari driver..." class="pl-10 pr-4 py-2 w-full sm:w-64 border rounded-lg shadow-sm
    ...
    @if(session('success'))
    <div
    class="p-4 rounded bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 text-green-800 dark:text-green-100 flex items-center gap-2 animate-fade-in">
    ...
    <table id="driverTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
      ...
      : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-100' }}">
        <span class="iconify"
        data-icon="{{ $driver->status === 'active' ? 'ph:check-circle' : 'ph:pause-circle' }}"></span>
        {{ ucfirst($driver->status) }}
        </span>
        </td>
        <td class="px-6 py-4 text-center space-x-2">
        <!-- Tombol Edit -->
        <a href="{{ route('admin.drivers.edit', $driver->id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-full
      bg-gradient-to-tr from-sky-500 to-blue-600
      shadow-md hover:shadow-lg hover:scale-110 transform
      transition duration-300 ease-in-out relative group" title="Edit">
        <!-- Icon Edit -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-800 dark:text-white" fill="none"
      ...
      bg-white text-gray-800 dark:bg-gray-800 dark:text-gray-100">
        ...
        @csrf
        @method('DELETE')
        <button type="submit" onclick="return confirm('Yakin ingin menghapus driver ini?')" class="inline-flex items-center justify-center w-10 h-10 rounded-full
         bg-gradient-to-tr from-rose-500 to-red-600
         shadow-md hover:shadow-lg hover:scale-110 transform
         transition duration-300 ease-in-out relative group" title="Hapus">
        <!-- Icon Delete -->
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-800 dark:text-white" fill="none"
          viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path d="M3 6h18" />
    ...
    @empty
      <tr>
      <td colspan="6" class="text-center px-6 py-4 text-gray-500 dark:text-gray-400">Belum ada driver terdaftar.
      </td>
      </tr>
    @endforelse
      </tbody>
    ...
    <form action="{{ route('admin.drivers.store') }}" method="POST">
      ...
      <div class="space-y-4">
        ...
        <input type="text" name="name" placeholder="Nama Lengkap" required
               class="w-full border rounded px-4 py-2 focus:ring-yellow-400 focus:border-yellow-400
